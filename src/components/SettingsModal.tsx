    ...
    >
      <div className="bg-slate-900 border border-slate-700 rounded-[2px] w-full max-w-2xl shadow-xl flex flex-col max-h-[90vh] overflow-hidden text-white">
        ...
        <div className="p-4 overflow-y-auto flex-1 flex flex-col gap-4 text-sm">
          ...
          {/* SECTION 6: AUTO NIGHT VISION & FLASHLIGHT AUTOMATION */}
          <div className="bg-slate-950 p-3.5 rounded-[2px] border border-slate-800 flex flex-col gap-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Moon className="w-4 h-4 text-indigo-400" />
                <h3 className="text-sm font-bold text-white">Auto Night Vision &amp; Flashlight</h3>
              </div>
              <span className="text-[10px] font-black uppercase bg-indigo-950 text-indigo-300 border border-indigo-500 px-2 py-0.5 rounded-[2px]">
                Optical Auto-Pilot
              </span>
            </div>
            ...
            <label className="flex items-center justify-between gap-2.5 cursor-pointer">
              <div className="flex items-center gap-2">
                <input
                  id="settings-auto-night-vision-checkbox"
                  type="checkbox"
                  checked={settings.autoNightVisionOnLowLight}
                  onChange={(e) => onUpdateSettings({ autoNightVisionOnLowLight: e.target.checked })}
                  className="w-4 h-4 rounded-[2px] accent-indigo-500 cursor-pointer"
                />
                <span className="text-slate-200 text-xs sm:text-sm">Auto Night Vision (IR mode) when room is dim</span>
              </div>
              <span className="text-xs text-indigo-400 font-mono font-bold shrink-0">
                &lt; {settings.lowLightThreshold || 35}% light
              </span>
            </label>
            ...
            <label className="flex items-center justify-between gap-2.5 cursor-pointer">
              <div className="flex items-center gap-2">
                <input
                  id="settings-auto-torch-checkbox"
                  type="checkbox"
                  checked={settings.autoTorchOnPitchDark}
                  onChange={(e) => onUpdateSettings({ autoTorchOnPitchDark: e.target.checked })}
                  className="w-4 h-4 rounded-[2px] accent-amber-500 cursor-pointer"
                />
                <span className="text-slate-200 text-xs sm:text-sm">Auto Flashlight (Torch) when pitch dark</span>
              </div>
              <span className="text-xs text-amber-400 font-mono font-bold shrink-0">
                &lt; {settings.pitchDarkThreshold || 15}% light
              </span>
            </label>
            ...
            <div className="grid grid-cols-2 gap-2 pt-2 border-t border-slate-800 text-xs">
              <div>
                <span className="text-slate-400 block mb-1">Low-Light Sensitivity:</span>
                <div className="flex gap-1">
                  {[25, 35, 45].map((thresh) => (
                    <button
                      key={thresh}
                      id={`low-light-thresh-${thresh}-btn`}
                      onClick={() => onUpdateSettings({ lowLightThreshold: thresh })}
                      className={`flex-1 py-1 text-center font-bold rounded-[2px] border transition ${
                        (settings.lowLightThreshold || 35) === thresh
              ...
              <div>
                <span className="text-slate-400 block mb-1">Pitch-Dark Threshold:</span>
                <div className="flex gap-1">
                  {[10, 15, 20].map((thresh) => (
                    <button
                      key={thresh}
                      id={`pitch-dark-thresh-${thresh}-btn`}
                      onClick={() => onUpdateSettings({ pitchDarkThreshold: thresh })}
                      className={`flex-1 py-1 text-center font-bold rounded-[2px] border transition ${
                        (settings.pitchDarkThreshold || 15) === thresh
