...
import { SeniorQuickGuideModal } from './SeniorQuickGuideModal';
import {
  AppSettings,
  BatteryState,
  CameraSlot,
...
export const CameraView: React.FC<CameraViewProps> = ({
  ...
  const [isScreenFlashlight, setIsScreenFlashlight] = useState(false);
  const [isNightMode, setIsNightMode] = useState(false);
  const [speakerAnnouncement, setSpeakerAnnouncement] = useState<string | null>(null);

  // Auto Low-Light, Night Vision & Flashlight Automation States
  const [ambientLuminance, setAmbientLuminance] = useState<number>(55);
  const [isLowLight, setIsLowLight] = useState(false);
  const [isPitchDark, setIsPitchDark] = useState(false);
  const [autoNightTriggered, setAutoNightTriggered] = useState(false);
  const [autoTorchTriggered, setAutoTorchTriggered] = useState(false);
  const [autoLightStatusMessage, setAutoLightStatusMessage] = useState<string | null>(null);
  const [isSeniorGuideOpen, setIsSeniorGuideOpen] = useState(false);

  const autoNightActiveRef = useRef<boolean>(false);
  const autoTorchActiveRef = useRef<boolean>(false);
  const lowLightConsecutiveCountRef = useRef<number>(0);
  const brightLightConsecutiveCountRef = useRef<number>(0);
  ...
  // Toggle Camera LED Flashlight / Torch with Screen Floodlight Fallback
  const toggleTorch = useCallback(async (forcedState?: boolean) => {
    try {
      const nextState = forcedState !== undefined ? forcedState : !isTorchOn;
      ...
      if (track) {
        ...
        if ('torch' in capabilities) {
          ...
          setIsScreenFlashlight(false);
          return;
        }
      }
      // If hardware torch is absent (front camera / laptop), light up the screen white
      setIsScreenFlashlight(nextState);
      setIsTorchOn(nextState);
    } catch (e) {
      console.warn('Hardware torch error, using screen backlight fallback:', e);
      const nextState = forcedState !== undefined ? forcedState : !isScreenFlashlight;
      setIsScreenFlashlight(nextState);
      setIsTorchOn(nextState);
    }
  }, [isTorchOn, isScreenFlashlight]);

  // Toggle Night Mode (IR / Low-Light AGC Enhancement)
  const toggleNightMode = useCallback((forcedState?: boolean) => {
    setIsNightMode((prev) => {
      const next = forcedState !== undefined ? forcedState : !prev;
      if (settings.seniorVoiceAlerts) {
        speakSeniorVoice(next ? 'Night vision activated.' : 'Night vision off.');
      }
      return next;
    });
  ...
  useEffect(() => {
    const unsubscribe = globalStreamChannel.onRemoteCommand((cmd) => {
      ...
      } else if (cmd.command === 'ANNOUNCE_SPEAKER') {
        // High-Priority Loud Speaker Announcement with Doorbell Chime & On-Screen Banner
        const payloadObj = cmd.payload as { message?: string } | string | undefined;
        const msgText = (typeof payloadObj === 'string' ? payloadObj : payloadObj?.message) || 'Announcement from caregiver';
        setSpeakerAnnouncement(msgText);
        speakAnnouncementOnSpeaker(msgText);
        setTimeout(() => setSpeakerAnnouncement(null), 9000);
        if (settings.autoAnswerIntercomEnabled || settings.seniorCareMode) {
          SeniorMonitorService.getInstance().setAutoAnswerActive(true);
          setSeniorHandsFreeStatus('Open Mic: Senior speaking hands-free...');
          setTimeout(() => {
      ...
      } else if (cmd.command === 'TOGGLE_TORCH') {
        toggleTorch();
      } else if (cmd.command === 'SET_TORCH') {
        const payloadObj = cmd.payload as { torchOn?: boolean } | boolean | undefined;
        const state = typeof payloadObj === 'object' && payloadObj !== null ? payloadObj.torchOn : Boolean(payloadObj);
        toggleTorch(state);
      } else if (cmd.command === 'TOGGLE_NIGHT_MODE') {
        toggleNightMode();
      } else if (cmd.command === 'SET_NIGHT_MODE') {
        const payloadObj = cmd.payload as { nightMode?: boolean } | boolean | undefined;
        const state = typeof payloadObj === 'object' && payloadObj !== null ? payloadObj.nightMode : Boolean(payloadObj);
        toggleNightMode(state);
      } else if (cmd.command === 'WALKIE_TALKIE_VOICE') {
        setIsIntercomReceiving(true);
        const payloadObj = cmd.payload as { audioBase64?: string; text?: string } | undefined;
        if (payloadObj?.audioBase64) {
          try {
            const audio = new Audio(payloadObj.audioBase64);
            audio.volume = 1.0;
            audio.play().catch(() => {
              if (payloadObj.text) speakSeniorVoice(payloadObj.text);
            });
          } catch {
            if (payloadObj.text) speakSeniorVoice(payloadObj.text);
          }
          ...
          playWalkieTalkieChirp();
          setTimeout(() => speakSeniorVoice(payloadObj.text!), 120);
        }
        if (settings.autoAnswerIntercomEnabled || settings.seniorCareMode) {
          SeniorMonitorService.getInstance().setAutoAnswerActive(true);
          setSeniorHandsFreeStatus('Open Mic: Senior speaking hands-free...');
    ...
    const processFrame = async () => {
      ...
      if (videoRef.current && now - lastAnalyzeTimeRef.current >= analyzeInterval) {
        ...
        const analysis = motionDetectorRef.current.analyzeFrame(
          ...
          settings.lowLightThreshold || 35,
          settings.pitchDarkThreshold || 15
        );

        setCurrentMotionScore(analysis.score);
        setAmbientLuminance(analysis.ambientLuminance);
        setIsLowLight(analysis.isLowLight);
        setIsPitchDark(analysis.isPitchDark);

        // Auto Night Vision (IR mode) on Low Light
        if (settings.autoNightVisionOnLowLight) {
          if (analysis.isLowLight) {
            lowLightConsecutiveCountRef.current++;
            brightLightConsecutiveCountRef.current = 0;
            if (lowLightConsecutiveCountRef.current >= 2 && !autoNightActiveRef.current) {
              autoNightActiveRef.current = true;
              setAutoNightTriggered(true);
              setIsNightMode(true);
              setAutoLightStatusMessage('Auto Low-Light: Night Vision (IR mode) engaged');
              if (settings.seniorVoiceAlerts) {
                speakSeniorVoice('Low light detected. Night vision activated.');
              }
            }
          } else if (analysis.ambientLuminance > (settings.lowLightThreshold || 35) + 12) {
            brightLightConsecutiveCountRef.current++;
            lowLightConsecutiveCountRef.current = 0;
            if (brightLightConsecutiveCountRef.current >= 4 && autoNightActiveRef.current) {
              autoNightActiveRef.current = false;
              setAutoNightTriggered(false);
              setIsNightMode(false);
              setAutoLightStatusMessage('Normal room light restored. Night vision disengaged.');
            }
          }
        }

        // Auto Flashlight / Torch on Pitch Dark
        if (settings.autoTorchOnPitchDark) {
          if (analysis.isPitchDark) {
            if (!autoTorchActiveRef.current) {
              autoTorchActiveRef.current = true;
              setAutoTorchTriggered(true);
              toggleTorch(true);
              setAutoLightStatusMessage('Pitch dark detected: Camera flashlight turned ON');
              if (settings.seniorVoiceAlerts) {
                speakSeniorVoice('Pitch dark detected. Flashlight turned on.');
              }
            }
          } else if (analysis.ambientLuminance > (settings.lowLightThreshold || 35) + 18) {
            if (autoTorchActiveRef.current) {
              autoTorchActiveRef.current = false;
              setAutoTorchTriggered(false);
              toggleTorch(false);
              setAutoLightStatusMessage('Room illuminated. Flashlight turned OFF.');
            }
          }
        }
  ...
  return (
    ...
    >
      ...
      {/* AUTO LIGHT STATUS BANNER */}
      {autoLightStatusMessage && (
        <div
          id="auto-light-camera-banner"
          className="bg-indigo-950 text-indigo-100 p-3 rounded-[2px] border-2 border-indigo-400 shadow-xl flex items-center justify-between gap-3"
        >
          <div className="flex items-center gap-2 font-bold text-sm">
            <Moon className="w-5 h-5 text-indigo-300 animate-pulse" />
            <span>{autoLightStatusMessage}</span>
          </div>
          <button
            onClick={() => setAutoLightStatusMessage(null)}
            className="text-xs bg-indigo-900 hover:bg-indigo-800 text-indigo-200 px-2 py-1 rounded-[2px] font-bold border border-indigo-400"
          >
            DISMISS
          </button>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {onOpenQRScanner && (
            <button
              ...
              className="ml-1 sm:ml-2 px-2.5 py-1 bg-emerald-600 hover:bg-emerald-500 text-slate-950 font-black text-xs rounded-[2px] border border-emerald-400 flex items-center gap-1.5 shadow transition"
              title="Scan Viewer QR Code to auto-pair"
            >
      ...
      <div className="relative bg-slate-950 rounded-[2px] overflow-hidden border-2 border-slate-700 shadow-xl min-h-[380px] sm:min-h-[460px] flex flex-col items-center justify-center">
        <video
          ...
          style={{
            filter: isNightMode
              ? 'grayscale(100%) contrast(175%) brightness(135%) drop-shadow(0 0 1px #22c55e)'
        ...
        {/* Screen Floodlight Overlay (Used when hardware LED torch is absent) */}
        {isScreenFlashlight && (
          <div
            id="camera-screen-flashlight-floodlight"
            onClick={() => toggleTorch(false)}
            className="fixed inset-0 z-50 bg-white flex flex-col items-center justify-center p-6 text-black select-none cursor-pointer"
          >
            <Sun className="w-20 h-20 text-amber-500 animate-pulse mb-3" />
            <h2 className="text-2xl sm:text-3xl font-black uppercase tracking-wider text-center">Screen Flashlight Active</h2>
            <p className="text-sm font-bold text-slate-800 mt-2 text-center">100% Maximum Brightness Room Floodlight</p>
            <button
              onClick={() => toggleTorch(false)}
              className="mt-6 px-6 py-3 bg-black text-white font-black text-sm rounded-[2px] border-2 border-slate-800 shadow"
            >
        ...
        {/* Loud Speaker Announcement Overlay */}
        {speakerAnnouncement && (
          <div
            id="camera-speaker-announcement-banner"
            className="absolute top-14 left-3 right-3 bg-emerald-950/95 border-2 border-emerald-400 p-3.5 rounded-[2px] text-emerald-200 text-sm font-black flex items-center gap-3 z-30 shadow-2xl animate-bounce"
          >
            <span className="p-2 bg-emerald-500 text-slate-950 text-xl rounded-[2px] shrink-0">📢</span>
            <div className="flex-1">
              <div className="text-[10px] text-emerald-300 uppercase tracking-wide font-black">Announcement on Camera Speaker:</div>
              <div className="text-white text-base font-black leading-snug">{speakerAnnouncement}</div>
            </div>
          </div>
        )}
        ...
        {isSurveillanceActive && (
          <>
            ...
            <div className="absolute top-3 left-3 right-3 flex flex-wrap items-center justify-between gap-2 pointer-events-none z-20">
              ...
              {/* Ambient Light HUD Meter */}
              <div
                id="camera-ambient-light-badge"
                className={`flex items-center gap-1.5 px-2.5 py-1 rounded-[2px] border text-xs font-bold shadow ${
                  isPitchDark
              ...
              >
                ...
                <span>
                  Light: {ambientLuminance}% {isPitchDark ? '(PITCH BLACK)' : isLowLight ? '(LOW LIGHT)' : ''}
                </span>
              </div>

              {/* Night Mode HUD Badge */}
              {isNightMode && (
                <div
                  id="camera-night-mode-badge"
                  className="flex items-center gap-1.5 bg-indigo-950/90 backdrop-blur px-2.5 py-1 rounded-[2px] border border-indigo-400 text-xs font-black text-indigo-200 shadow"
                >
                  <Moon className="w-3.5 h-3.5 text-indigo-400" />
                  <span>Night Vision ON {autoNightTriggered ? '(AUTO)' : ''}</span>
                </div>
              )}

              {/* Flashlight HUD Badge */}
              {isTorchOn && (
                <div
                  id="camera-flashlight-badge"
                  className="flex items-center gap-1.5 bg-amber-950/90 backdrop-blur px-2.5 py-1 rounded-[2px] border border-amber-400 text-xs font-black text-amber-200 shadow"
                >
                  <Sun className="w-3.5 h-3.5 text-amber-400 animate-pulse" />
                  <span>Flashlight ON {autoTorchTriggered ? '(AUTO)' : ''}</span>
                </div>
              )}
      ...
      {/* SENIOR LIVING AUTONOMOUS STATION (LIABILITY-SAFE) */}
      <div
        id="senior-autonomous-station-card"
        className="bg-slate-900/95 border-2 border-slate-700 rounded-[2px] p-4 flex flex-col gap-3 shadow-xl"
      >
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border-b border-slate-800 pb-3">
          <div className="flex items-center gap-2.5">
            <div className="w-9 h-9 rounded-[2px] bg-slate-800 border border-slate-700 flex items-center justify-center text-emerald-400 shrink-0">
              <Shield className="w-5 h-5" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-black text-slate-200 tracking-wide">
                  CAMERA STATION ACTIVE
                </span>
                <span className="bg-slate-800 text-slate-400 border border-slate-700 px-1.5 py-0.5 rounded-[2px] text-[10px] font-mono">
                  AUTOPILOT
                </span>
              </div>
              <p className="text-xs text-slate-400 font-medium">
                Keep phone connected to charger. Zero buttons required to operate.
              </p>
            </div>
          </div>

          <button
            id="senior-open-guide-btn"
            onClick={() => setIsSeniorGuideOpen(true)}
            className="px-3 py-2 bg-slate-800 hover:bg-slate-700 text-slate-200 hover:text-white font-bold text-xs rounded-[2px] border border-slate-600 flex items-center gap-1.5 transition shadow self-end sm:self-auto"
            title="Open senior user guide"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
          <div className="p-2.5 bg-slate-950 border border-slate-800 rounded-[2px] flex items-center gap-2.5">
            <Mic className="w-4 h-4 text-emerald-400 shrink-0 animate-pulse" />
            <div className="flex-1 min-w-0">
              <span className="block font-bold text-slate-300 truncate">Hands-Free Mic Ear Active</span>
              <span className="text-[11px] text-slate-400 truncate">Listens for distress phrase: &quot;Help me&quot;</span>
            </div>
          </div>
          ...
          <div className="p-2.5 bg-slate-950 border border-slate-800 rounded-[2px] flex items-center gap-2.5">
            ...
            <div className="flex-1 min-w-0">
              <span className="block font-bold text-slate-300 truncate">Continuous Power Guard</span>
              <span className="text-[11px] text-slate-400 truncate">Operates 24/7 on wall charger or backup battery</span>
            </div>
          </div>
        </div>

        {/* BIG EMERGENCY HELP / SOS BUTTON (CALL FAMILY) */}
        <div className="p-3 bg-red-950/40 border border-red-500/60 rounded-[2px] flex flex-col sm:flex-row items-center justify-between gap-3">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <div className="text-white font-black text-sm">Need family or caregiver assistance?</div>
              <div className="text-[11px] text-slate-300">
                Say <strong className="text-amber-300">&quot;Help me&quot;</strong> aloud, or press this button to alert family:
              </div>
            </div>
          </div>

          <button
            id="camera-view-emergency-sos-btn"
            onClick={() => {
              SeniorMonitorService.getInstance().triggerVoiceDistressAlert('Senior pressed Emergency SOS button on camera phone');
            }}
            className="w-full sm:w-auto px-5 py-2.5 bg-red-600 hover:bg-red-500 active:bg-red-700 text-white font-black text-xs rounded-[2px] border border-red-300 shadow flex items-center justify-center gap-2 transition active:scale-95 shrink-0"
          >
        ...
        <div className="text-[10px] text-slate-400 text-center leading-relaxed px-1">
          ⚠️ <em>Informational DIY tool for family awareness only. Not a certified alarm, fire detector, or 911 dispatch service. In an emergency, dial 911.</em>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
        {isSurveillanceActive ? (
          <button
            id="stop-camera-btn"
            onClick={stopCamera}
            className="py-4 px-4 bg-red-600 hover:bg-red-500 active:bg-red-700 text-white font-black text-sm rounded-[2px] shadow flex items-center justify-center gap-2 transition border-2 border-red-400"
          >
          ...
          <button
            ...
            onClick={startCamera}
            className="py-4 px-4 bg-emerald-600 hover:bg-emerald-500 active:bg-emerald-700 text-white font-black text-sm rounded-[2px] shadow flex items-center justify-center gap-2 transition border-2 border-emerald-400"
          >
        ...
        <button
          id="camera-flashlight-toggle-btn"
          onClick={() => toggleTorch()}
          className={`py-4 px-4 font-black text-sm rounded-[2px] shadow flex items-center justify-center gap-2 transition border-2 ${
            isTorchOn
              ? 'bg-amber-500 hover:bg-amber-400 text-slate-950 border-amber-300 ring-2 ring-amber-300'
              : 'bg-slate-800 hover:bg-slate-700 text-white border-slate-600'
          }`}
          title="Toggle camera phone LED flashlight (with screen floodlight fallback)"
        >
          <Sun className={`w-5 h-5 ${isTorchOn ? 'text-slate-950' : 'text-amber-400'}`} />
          <span>{isTorchOn ? 'LIGHT: ON' : 'FLASHLIGHT'}</span>
        </button>

        {/* NIGHT MODE / IR VISION CONTROL */}
        <button
          id="camera-nightmode-toggle-btn"
          onClick={() => toggleNightMode()}
          className={`py-4 px-4 font-black text-sm rounded-[2px] shadow flex items-center justify-center gap-2 transition border-2 ${
            isNightMode
              ? 'bg-indigo-600 hover:bg-indigo-500 text-white border-indigo-300 ring-2 ring-indigo-300'
              : 'bg-slate-800 hover:bg-slate-700 text-white border-slate-600'
          }`}
          title="Toggle infrared monochrome AGC Night Mode for dark rooms"
        >
          <Moon className={`w-5 h-5 ${isNightMode ? 'text-indigo-200' : 'text-slate-400'}`} />
          <span>{isNightMode ? 'NIGHT VISION: ON' : 'NIGHT MODE'}</span>
        </button>
        ...
        <button
          ...
          className="py-4 px-4 bg-slate-800 hover:bg-slate-700 active:bg-slate-600 text-white font-black text-sm rounded-[2px] shadow flex items-center justify-center gap-2 transition border-2 border-slate-600"
        >
      ...
      {/* AUTO NIGHT VISION & FLASHLIGHT AUTOMATION CARD */}
      <div className="bg-slate-900 border border-slate-700 rounded-[2px] p-4 sm:p-5 flex flex-col gap-4">
        <div className="flex flex-wrap items-center justify-between gap-2.5 border-b border-slate-800 pb-3">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-lg sm:text-xl font-black text-white flex items-center gap-2">
                <span>AUTOMATIC NIGHT VISION &amp; FLASHLIGHT</span>
                <span className="text-xs bg-indigo-500/20 text-indigo-300 border border-indigo-400 px-2 py-0.5 rounded-[2px] font-black uppercase">
                  Optical Auto-Pilot
                </span>
              </h3>
              <p className="text-xs text-slate-300 font-medium">
                Automatic ambient luminance detection — engages IR Night Mode at dusk and turns on LED Flashlight in pitch darkness
              </p>
            </div>
          </div>
          {/* Live Ambient Luminance Bar */}
          <div className="flex items-center gap-2 bg-slate-950 px-3 py-1.5 rounded-[2px] border border-slate-800">
            <span className="text-xs text-slate-400 font-bold">Room Light:</span>
            <span className="text-sm font-mono font-black text-amber-300">{ambientLuminance}%</span>
            <span
            ...
            >
              {isPitchDark ? 'Pitch Dark' : isLowLight ? 'Low Light' : 'Normal'}
            </span>
          </div>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {/* Auto Night Vision Toggle */}
          <div className="bg-slate-950 p-3.5 rounded-[2px] border border-slate-800 flex flex-col justify-between gap-3">
            <div className="flex items-start justify-between gap-2">
              <div>
                <div className="text-sm font-black text-white flex items-center gap-1.5">
                  <Moon className="w-4 h-4 text-indigo-400" />
                  Auto Night Vision (Low Light)
                </div>
                <div className="text-xs text-slate-400 mt-1">
                  Switches camera to high-contrast IR monochrome mode when room luminance drops below {settings.lowLightThreshold || 35}%.
                </div>
              </div>
              <button
                id="toggle-auto-night-vision-btn"
                onClick={() =>
                  onUpdateSettings({ autoNightVisionOnLowLight: !settings.autoNightVisionOnLowLight })
                }
                className={`px-3 py-1.5 rounded-[2px] text-xs font-black uppercase border shrink-0 transition ${
                  settings.autoNightVisionOnLowLight
                    ? 'bg-indigo-600 text-white border-indigo-400 shadow'
                    : 'bg-slate-800 text-slate-400 border-slate-700'
                }`}
              >
                {settings.autoNightVisionOnLowLight ? 'AUTO: ON' : 'OFF'}
              </button>
            </div>
            {settings.autoNightVisionOnLowLight && autoNightTriggered && (
              <div className="text-[11px] font-bold text-indigo-300 bg-indigo-950/60 p-1.5 rounded-[2px] border border-indigo-800 flex items-center gap-1.5">
                <CheckCircle2 className="w-3.5 h-3.5 text-indigo-400 shrink-0" />
                <span>Currently active due to low ambient light ({ambientLuminance}%)</span>
              </div>
            )}
          </div>

          {/* Auto Flashlight / Torch Toggle */}
          <div className="bg-slate-950 p-3.5 rounded-[2px] border border-slate-800 flex flex-col justify-between gap-3">
            <div className="flex items-start justify-between gap-2">
              <div>
                <div className="text-sm font-black text-white flex items-center gap-1.5">
                  <Sun className="w-4 h-4 text-amber-400" />
                  Auto Flashlight (Pitch Dark)
                </div>
                <div className="text-xs text-slate-400 mt-1">
                  Turns on camera LED torch (or screen floodlight) when room luminance drops below {settings.pitchDarkThreshold || 15}%.
                </div>
              </div>
              <button
                id="toggle-auto-torch-btn"
                onClick={() =>
                  onUpdateSettings({ autoTorchOnPitchDark: !settings.autoTorchOnPitchDark })
                }
                className={`px-3 py-1.5 rounded-[2px] text-xs font-black uppercase border shrink-0 transition ${
                  settings.autoTorchOnPitchDark
                    ? 'bg-amber-500 text-slate-950 border-amber-300 shadow font-black'
                    : 'bg-slate-800 text-slate-400 border-slate-700'
                }`}
              >
                {settings.autoTorchOnPitchDark ? 'AUTO: ON' : 'OFF'}
              </button>
            </div>
            {settings.autoTorchOnPitchDark && autoTorchTriggered && (
              <div className="text-[11px] font-bold text-amber-300 bg-amber-950/60 p-1.5 rounded-[2px] border border-amber-800 flex items-center gap-1.5">
                <CheckCircle2 className="w-3.5 h-3.5 text-amber-400 shrink-0" />
                <span>Currently active due to pitch dark room ({ambientLuminance}%)</span>
              </div>
            )}
          </div>
        </div>
      ...
      <SeniorQuickGuideModal
        ...
        onTriggerSOS={() => {
          SeniorMonitorService.getInstance().triggerVoiceDistressAlert('Senior pressed Emergency SOS button');
        }}
      />
    </div>
  );
