...
export const MonitorView: React.FC<MonitorViewProps> = ({
  ...
  const [announceTarget, setAnnounceTarget] = useState<CameraSlot | 'all'>('all');
  const [connectionInfo, setConnectionInfo] = useState<StreamConnectionState>(globalStreamChannel.getConnectionState());
  ...
  useEffect(() => {
    ...
    const unsubCmd = globalStreamChannel.onRemoteCommand((cmd: any) => {
      ...
      } else if (cmd.command === 'VOICE_SOS_TRIGGERED') {
        playEmergencyAlarmSiren(3);
        const spoken = `EMERGENCY! Senior Voice SOS triggered: "${cmd.payload?.phrase || 'Help'}"! Check camera feed immediately!`;
        speakSeniorVoice(spoken);
        setLastMotionAlert({
          cameraId: cmd.payload?.cameraId || 'cam1',
          cameraName: cmd.payload?.cameraName || 'Senior Room Camera',
          time: cmd.timestamp || Date.now(),
          score: 100,
          type: 'voice_help',
        });
  ...
  // Toggle Camera Flashlight / LED Torch
  const handleToggleTorch = (target: CameraSlot | 'all' = 'all') => {
    globalStreamChannel.sendRemoteCommand('TOGGLE_TORCH', target);
    setCamerasState((prev) => {
      const next = { ...prev };
      if (target === 'all') {
        (Object.keys(next) as CameraSlot[]).forEach((key) => {
          if (next[key]) {
            next[key] = { ...next[key]!, torchOn: !next[key]!.torchOn };
          }
        });
      } else if (next[target]) {
        next[target] = { ...next[target]!, torchOn: !next[target]!.torchOn };
      }
      return next;
    });
    if (settings.seniorVoiceAlerts) {
      speakSeniorVoice('Toggled camera flashlight.');
    }
  };

  // Toggle Camera IR AGC Night Mode
  const handleToggleNightMode = (target: CameraSlot | 'all' = 'all') => {
    globalStreamChannel.sendRemoteCommand('TOGGLE_NIGHT_MODE', target);
    setCamerasState((prev) => {
      const next = { ...prev };
      if (target === 'all') {
        (Object.keys(next) as CameraSlot[]).forEach((key) => {
          if (next[key]) {
            next[key] = { ...next[key]!, nightMode: !next[key]!.nightMode };
          }
        });
      } else if (next[target]) {
        next[target] = { ...next[target]!, nightMode: !next[target]!.nightMode };
      }
      return next;
    });
    if (settings.seniorVoiceAlerts) {
      speakSeniorVoice('Toggled camera night vision.');
    }
  };

  // Send Loud Speaker Announcement with Doorbell Chime
  const handleSendAnnouncement = (msg: string, target: CameraSlot | 'all' = 'all') => {
    if (!msg.trim()) return;
    playDoorbellChime();
    globalStreamChannel.sendRemoteCommand('ANNOUNCE_SPEAKER', target, { message: msg.trim() });
    if (settings.seniorVoiceAlerts) {
      speakSeniorVoice(`Announcement sent to camera speaker: ${msg.trim()}`);
    }
  };

  // Walkie-Talkie Push-to-Talk with Live Microphone Recording
  const handleStartTalking = async (target: CameraSlot | 'all' = 'all') => {
    setIsTalking(true);
    playWalkieTalkieChirp();
    ...
    try {
      if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
        ...
        recorder.onstop = () => {
          ...
          reader.onloadend = () => {
            ...
            globalStreamChannel.sendVoiceAudio(base64, target, customWalkieMsg || undefined);
          };
      ...
      console.warn('Microphone access or MediaRecorder unavailable:', err);
    }
  };
  ...
  return (
    <div className="flex flex-col gap-4 select-none">
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-[2px] p-3.5 sm:p-4 shadow flex flex-col gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2 w-full md:w-auto justify-end">
            {onOpenPairingQR && (
              <button
                ...
                onClick={() => onOpenPairingQR(activeCameraId)}
                className="px-3 py-1.5 rounded-[2px] font-black text-xs flex items-center gap-1.5 bg-emerald-600 hover:bg-emerald-500 text-slate-950 border border-emerald-400 shadow transition"
                title="Generate instant QR code for old phone to scan and pair"
              >
                <QrCode className="w-3.5 h-3.5" />
                <span>+ PAIR CAMERA QR</span>
              </button>
            )}
            <button
              id="layout-grid-btn"
        ...
        {/* CLOUD BROKER SYNC & REMOTE QUICK ACTIONS BAR */}
        <div className="pt-2 border-t border-slate-800 flex flex-col md:flex-row items-start md:items-center justify-between gap-2 text-xs">
          <div className="flex flex-wrap items-center gap-2">
            <div className="flex items-center gap-1.5 bg-slate-950 px-2 py-1 rounded-[2px] border border-slate-800">
              <span className={`w-2 h-2 rounded-full ${connectionInfo.connected ? 'bg-emerald-400 animate-ping' : 'bg-amber-400'}`} />
              <span className="font-bold text-slate-200">
                {connectionInfo.connected ? 'Cloud Cross-Device Sync:' : 'Syncing Cloud:'}
              </span>
              <span className="text-emerald-400 font-mono font-bold">
                {connectionInfo.connected ? 'ONLINE (MQTT)' : 'LOCAL ONLY'}
              </span>
            </div>

            {userEmail && (
              <div className="flex items-center gap-1 bg-slate-950 px-2 py-1 rounded-[2px] border border-slate-800 text-slate-300">
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            <button
              onClick={() => handleToggleTorch('all')}
              className="px-2.5 py-1 bg-slate-800 hover:bg-slate-700 text-amber-300 font-bold rounded-[2px] border border-slate-700 hover:border-amber-400 flex items-center gap-1 transition"
              title="Toggle flashlight on all connected cameras"
            >
              <Sun className="w-3.5 h-3.5" />
              <span>Torch All</span>
            </button>
            <button
              onClick={() => handleToggleNightMode('all')}
              className="px-2.5 py-1 bg-slate-800 hover:bg-slate-700 text-emerald-300 font-bold rounded-[2px] border border-slate-700 hover:border-emerald-400 flex items-center gap-1 transition"
              title="Toggle night vision mode on all cameras"
            >
              <Moon className="w-3.5 h-3.5" />
              <span>Night Mode All</span>
            </button>
            <button
              onClick={() => {
              ...
              className="px-2.5 py-1 bg-emerald-700 hover:bg-emerald-600 text-white font-bold rounded-[2px] border border-emerald-500 flex items-center gap-1 transition shadow"
              title="Broadcast voice announcement to all camera speakers"
            >
              <Volume2 className="w-3.5 h-3.5" />
              <span>Announce All</span>
            </button>
          </div>
        </div>
      </div>
      ...
      {viewLayout === 'grid' ? (
          ...
          {CAMERA_SLOTS.map((slot) => {
            ...
            return (
              ...
              >
                <div className="aspect-video relative flex items-center justify-center bg-slate-900 overflow-hidden">
                  {cam?.currentFrame ? (
                    <img
                      ...
                      alt={slot.name}
                      style={{
                        filter: cam.nightMode
                          ? 'grayscale(100%) contrast(175%) brightness(135%)'
                  ...
                  {/* Flashlight Active Badge */}
                  {cam?.torchOn && (
                    <div className="absolute top-8 left-2 bg-amber-400 text-slate-950 font-black text-[10px] px-1.5 py-0.5 rounded-[2px] flex items-center gap-1 z-10 shadow animate-pulse border border-white">
                  ...
                  {cam?.nightMode && (
                    <div className="absolute top-8 right-2 bg-emerald-500 text-slate-950 font-black text-[10px] px-1.5 py-0.5 rounded-[2px] flex items-center gap-1 z-10 shadow border border-white">
                ...
                <div className="p-2.5 bg-slate-900 border-t border-slate-800 flex flex-wrap items-center justify-between text-xs gap-2">
                  <div className="flex flex-col truncate">
                    <span className="text-slate-300 font-bold truncate">{slot.location}</span>
                    <div className="flex items-center gap-1.5 text-[10px]">
                      <span className="text-emerald-400 font-mono">Bat: {cam?.battery?.level || 80}%</span>
                      {cam?.ambientLuminance !== undefined && (
                        <span className="text-amber-300 font-mono">
                          • Light: {cam.ambientLuminance}%{cam.isPitchDark ? ' (Dark)' : cam.isLowLight ? ' (Low)' : ''}
                        </span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        handleToggleTorch(slot.id);
                      }}
                      className={`p-1.5 rounded-[2px] text-xs font-bold border transition ${
                        cam?.torchOn
                          ? 'bg-amber-400 text-slate-950 border-amber-300'
                          : 'bg-slate-800 hover:bg-slate-750 text-slate-300 border-slate-700'
                      }`}
                      title="Toggle Flashlight on this camera"
                    >
                      <Sun className="w-3.5 h-3.5" />
                    </button>
                    <button
                      ...
                      title="Toggle Night Vision on this camera"
                    >
                      <Moon className="w-3.5 h-3.5" />
                    </button>
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        setAnnounceTarget(slot.id);
                        setIsAnnounceModalOpen(true);
                      }}
                      className="p-1.5 bg-slate-800 hover:bg-slate-750 text-emerald-300 rounded-[2px] border border-slate-700 hover:border-emerald-500 transition"
                      title="Send voice announcement to this camera speaker"
                    >
                      <Volume2 className="w-3.5 h-3.5" />
                    </button>
                    {onOpenPairingQR && (
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          onOpenPairingQR(slot.id);
                        }}
                        className="p-1.5 bg-slate-800 hover:bg-slate-750 text-amber-300 rounded-[2px] border border-slate-700 hover:border-amber-400 transition"
                        title={`Show instant pairing QR for ${slot.name}`}
                      >
                        <QrCode className="w-3.5 h-3.5" />
                      </button>
                    )}
                    <button
                      ...
                      className="px-2 py-1 bg-emerald-600 hover:bg-emerald-500 text-white font-black text-[11px] rounded-[2px] flex items-center gap-1 transition shadow"
        ...
        <div className="flex flex-col gap-3">
          <div className="relative bg-slate-950 rounded-[2px] overflow-hidden border-2 border-slate-700 shadow-xl min-h-[340px] sm:min-h-[440px] flex items-center justify-center">
            ...
            >
              {activeCamStatus?.currentFrame ? (
                <img
                  ...
                  style={{
                    filter: activeCamStatus.nightMode
                      ? 'grayscale(100%) contrast(175%) brightness(135%)'
                      : activeCamStatus.privacyShieldActive
                      ? 'blur(24px)'
                      : 'none',
                  }}
            ...
            {/* Top Live Status Bar and Top Right Badges */}
            <div className="absolute top-3 left-3 right-3 flex flex-wrap items-center justify-between gap-2 pointer-events-none z-20">
              <div className="flex items-center gap-2 bg-slate-900/90 backdrop-blur px-2.5 py-1 rounded-[2px] border border-slate-700 text-white font-black text-xs pointer-events-auto shadow">
                <span className="w-2 h-2 rounded-[2px] bg-emerald-500 animate-ping" />
                <span className="truncate max-w-[140px] sm:max-w-none">LIVE • {activeCamConfig?.name}</span>
                <span className="text-slate-400">|</span>
                <span className="text-amber-400">{activeCamStatus?.fps || 15} FPS</span>
                <span className="text-slate-400 hidden sm:inline">|</span>
                <span className="text-cyan-300 font-mono hidden sm:inline">1080p</span>
              </div>

              {/* Top Right Zoom & AI Frame & Remote State Badges */}
              <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 pointer-events-auto ml-auto">
                {activeCamStatus?.ambientLuminance !== undefined && (
                  <div
                    className={`backdrop-blur px-2 sm:px-2.5 py-1 rounded-[2px] border text-xs font-bold flex items-center gap-1.5 shadow ${
                      activeCamStatus.isPitchDark
                        ? 'bg-purple-950/90 border-purple-400 text-purple-200'
                        : activeCamStatus.isLowLight
                  ...
                  >
                    ...
                    <span>
                      {activeCamStatus.ambientLuminance}%
                      <span className="hidden sm:inline">
                        {activeCamStatus.isPitchDark ? ' (Dark)' : activeCamStatus.isLowLight ? ' (Low)' : ''}
                      </span>
                    </span>
                  </div>
                )}
                {activeCamStatus?.torchOn && (
                  <div className="bg-amber-400 text-slate-950 font-black text-xs px-2 py-1 rounded-[2px] flex items-center gap-1 shadow animate-pulse border border-white">
                    <Sun className="w-3.5 h-3.5 animate-spin" />
                    <span>TORCH ON</span>
                  </div>
                )}
                {activeCamStatus?.nightMode && (
                  <div className="bg-emerald-500 text-slate-950 font-black text-xs px-2 py-1 rounded-[2px] flex items-center gap-1 shadow border border-white">
                    <Moon className="w-3.5 h-3.5" />
                    <span>IR NIGHT</span>
                  </div>
                )}
                <div className="bg-slate-900/90 backdrop-blur px-2 sm:px-2.5 py-1 rounded-[2px] border border-slate-700 text-amber-300 font-bold text-xs flex items-center gap-1.5">
                  <ZoomIn className="w-3.5 h-3.5" />
                  <span>{zoomLevel.toFixed(1)}x</span>
                </div>
                {settings.showTimestamp && (
                  <div className="hidden md:flex bg-black/80 font-mono text-emerald-400 px-2.5 py-1 rounded-[2px] text-xs border border-slate-800">
          ...
          <div className="bg-slate-900 border border-slate-700 p-3 rounded-[2px] flex flex-wrap items-center justify-between gap-3">
            ...
            {/* Quick Remote Camera Tools (Torch, Night Vision, Announce, Talk) */}
            <div className="flex flex-wrap items-center gap-2">
              <button
                id="toggle-focused-torch-btn"
                onClick={() => handleToggleTorch(activeCameraId)}
                className={`px-3 py-1.5 rounded-[2px] text-xs font-bold border transition flex items-center gap-1.5 ${
                  activeCamStatus?.torchOn
                    ? 'bg-amber-400 text-slate-950 border-amber-300 shadow'
                    : 'bg-slate-800 hover:bg-slate-750 text-slate-300 border-slate-700'
                }`}
                title="Toggle camera flashlight / torch remotely"
              >
                <Sun className="w-3.5 h-3.5" />
                <span>{activeCamStatus?.torchOn ? 'Torch: ON' : 'Torch: OFF'}</span>
              </button>

              <button
                id="toggle-focused-night-mode-btn"
                onClick={() => handleToggleNightMode(activeCameraId)}
                className={`px-3 py-1.5 rounded-[2px] text-xs font-bold border transition flex items-center gap-1.5 ${
                  activeCamStatus?.nightMode
                    ? 'bg-emerald-500 text-slate-950 border-emerald-300 shadow'
                    : 'bg-slate-800 hover:bg-slate-750 text-slate-300 border-slate-700'
                }`}
                title="Toggle camera infrared night vision remotely"
              >
                <Moon className="w-3.5 h-3.5" />
                <span>{activeCamStatus?.nightMode ? 'Night Vision: ON' : 'Night Vision: OFF'}</span>
              </button>

              <button
                id="focused-announce-btn"
                onClick={() => {
                  setAnnounceTarget(activeCameraId);
                  setIsAnnounceModalOpen(true);
                }}
                className="px-3 py-1.5 bg-slate-800 hover:bg-slate-750 text-emerald-300 font-bold text-xs rounded-[2px] border border-slate-700 hover:border-emerald-500 flex items-center gap-1.5 transition"
                title="Send speaker announcement"
              >
                <Volume2 className="w-3.5 h-3.5" />
                <span>Announce</span>
              </button>
              ...
              {onOpenPairingQR && (
                <button
                  id="focused-pairing-qr-btn"
                  onClick={() => onOpenPairingQR(activeCameraId)}
                  className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-slate-950 font-black text-xs rounded-[2px] border border-emerald-400 flex items-center gap-1.5 shadow transition"
                  title="Generate instant QR code for this slot"
                >
                  <QrCode className="w-3.5 h-3.5" />
                  <span>Pair QR</span>
                </button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
        ...
        <button
          ...
          className={`py-4 px-4 font-black text-base rounded-[2px] shadow flex items-center justify-center gap-2.5 transition active:scale-95 border-2 ${
            sirenActive
        ...
        >
          <ShieldAlert className={`w-6 h-6 shrink-0 ${sirenActive ? 'animate-bounce' : ''}`} />
          <div className="flex flex-col text-left leading-tight">
            <span>{sirenActive ? 'ALARM RINGING' : 'TRIGGER ALARM'}</span>
            <span className="text-[10px] text-red-200 font-normal">Audible Siren on Cameras</span>
          </div>
        </button>
        ...
        >
          <Radio className="w-6 h-6 shrink-0 text-emerald-200" />
          <div className="flex flex-col text-left leading-tight">
            <span>{isTalking ? 'TALKING...' : 'WALKIE TALKIE'}</span>
            <span className="text-[10px] text-emerald-200 font-normal">Push-To-Talk Button</span>
          </div>
        </button>

        {/* SPEAKER ANNOUNCEMENT MODAL BUTTON */}
        <button
          id="viewer-speaker-announcement-btn"
          onClick={() => {
            setAnnounceTarget(activeCameraId || 'all');
            setIsAnnounceModalOpen(true);
          }}
          className="py-4 px-4 bg-teal-700 hover:bg-teal-600 active:bg-teal-800 text-white font-black text-base rounded-[2px] shadow flex items-center justify-center gap-2.5 transition active:scale-95 border-2 border-teal-400"
          title="Broadcast loud speech announcement through camera speaker"
        >
          <Volume2 className="w-6 h-6 shrink-0 text-teal-200" />
          <div className="flex flex-col text-left leading-tight">
            <span>ANNOUNCE</span>
            <span className="text-[10px] text-teal-200 font-normal">Loud Speaker Broadcast</span>
          </div>
        </button>
        ...
        >
          <Video className="w-6 h-6 shrink-0 text-amber-400" />
          <div className="flex flex-col text-left leading-tight">
        ...
        <button
          ...
          onClick={onOpenShare}
          className="py-4 px-4 bg-amber-500 hover:bg-amber-400 active:bg-amber-600 text-slate-950 font-black text-base rounded-[2px] shadow flex items-center justify-center gap-2.5 transition active:scale-95 border-2 border-amber-300"
          title="Share viewer link with friends or relatives"
        >
          <Share2 className="w-6 h-6 shrink-0" />
          <div className="flex flex-col text-left leading-tight">
            <span>SHARE LINK</span>
            <span className="text-[10px] text-slate-900 font-bold">Relatives / Stations</span>
          </div>
        </button>
      </div>
