...
export interface AppSettings {
  ...
  // Auto Low-Light, Night Vision & Torch Options
  autoNightVisionOnLowLight: boolean; // Auto engage IR night vision when room ambient luminance drops
  autoTorchOnPitchDark: boolean; // Auto turn on physical camera LED torch/floodlight in pitch black
  lowLightThreshold: number; // Luminance threshold (0-100, default ~35)
  pitchDarkThreshold: number; // Pitch dark threshold (0-100, default ~18)
...
export interface CameraStatusBroadcast {
  ...
  torchOn?: boolean;
  nightMode?: boolean;
  // Ambient Low-Light and Auto-Detection Telemetry
  ambientLuminance?: number; // 0 (black) to 100 (bright)
  isLowLight?: boolean;
  isPitchDark?: boolean;
