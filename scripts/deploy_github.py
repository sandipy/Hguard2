...
def main():
    ...
    # 7. Ensure GitHub Pages is active via GitHub REST API
    if token:
        try:
            import urllib.request
            import json
            req = urllib.request.Request(
                f"https://api.github.com/repos/sandipy/{repo_name}/pages",
                data=json.dumps({
                    "source": {
                        "branch": "gh-pages",
                        "path": "/"
                ...
                headers={
                    ...
                    "User-Agent": "HGuard-Deploy"
                },
                method="POST"
            )
            with urllib.request.urlopen(req) as response:
                print(f"--> GitHub Pages setup response: {response.status}")
        except Exception as pe:
            print(f"--> GitHub Pages status notice: {pe}")
